...
export class FeatureModalComponent implements OnInit {
  ...
  getMinValue(): number {
    return Math.min(...this.objectKeys(this.keywords).map(key => this.getKeywordValue(key)));
  }

  getMaxValue(): number {
    return Math.max(...this.objectKeys(this.keywords).map(key => this.getKeywordValue(key)));
  }
